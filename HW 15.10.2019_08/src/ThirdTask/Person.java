...
public class Person {
	...
	protected Person(String Fname,String Lname,String ex,String job,String[] date,float height,float weight,float finalGrade,
			...
			String country,String city,String municipality,int postalCode,String street,int number,int floor,int apartmentNo) {
		this.Fname=Fname;
		this.Lname=Lname;
		this.ex=ex;
		this.job=job;
		...
		this.ad=new Address(country,city,municipality,postalCode,street,number,floor,apartmentNo);
	}
		...
		if(ChronoUnit.YEARS.between(date,LocalDate.now())>=18) {
			if(this.ex.equals("F")) {
				return this.Fname+" "+this.Lname + " is " + ChronoUnit.YEARS.between(date,LocalDate.now())+ " years old. Her weight is " + this.weight +  
					" and she is "+this.height+" cm tall. She is a "+this.job +"."+"She is under-aged.She started "+" \n "+this.e.toString(type)
					+this.e.getDegree()+" \n"+this.ad.toString();
			}else {
